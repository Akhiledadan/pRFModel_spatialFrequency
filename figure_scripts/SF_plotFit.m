function SF_plotFit(params_comp,opt,dirPth)
...
for cond_idx = 1:numCond
    ...
    for roi_idx = 1:numRoi
        ...
        figure(2); hold on;
        plot(params_comp.fit.x_fit,params_comp.fit.yfit_comp{cond_idx,roi_idx},'color',color_map(roi_idx,:),'MarkerSize',10, 'linewidth',3);
...
%    figure(31); ax31 = subplot(numRows,numCols,roi_idx);
    ...
    for cond_idx = 1:numCond
        ...
        figure(3); hold on;
        plot(params_comp.fit.x_fit,params_comp.fit.yfit_comp{cond_idx,roi_idx},'color',color_map(cond_idx,:),'MarkerSize',10,'linewidth',3);
        xlabel('eccentricity (deg)'); ylabel('pRF size (deg)');
        ylim([0 2]);
        xlim(opt.xaxislim);
