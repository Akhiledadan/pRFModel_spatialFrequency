...
commandLine <- sprintf( '3dSurf2Vol -spec surfaces_folder/spec.surfaces.smoothed -surf_A surfaces_folder/%s_sm.1D.coord -sv %s -grid_parent %s -map_func mask -prefix %s -sdata_1D %s', args[2], args[3], args[3], 'surfVolWMRoi', args[1] )
print( commandLine )
system( commandLine )
