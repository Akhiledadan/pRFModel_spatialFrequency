function SF_stats(paramsToCompare,paramsToCompare_type,rois,stats_folder)
% SF_stats - repeated measures anova to compare auc or
% central values of pRF size between different conditions for same subject

% folder containing results
if ~exist('stats_folder','var')
    stats_folder = '/home/edadan/SF/code/pRFModel_spatialFrequency/data/MS/stats_results';
end
...
if strcmpi(paramsToCompare_type,'absolute') % If the absolute values are used for ANOVA. Difference between the means/std of two distribution are compared
    ...
    if strcmpi(paramsToCompare,'central')
        filename_res = sprintf('cen_auc.mat');
        load(fullfile(stats_folder,filename_res));
        
        param = params_comp_all.cen;
        
    elseif strcmpi(paramsToCompare,'auc')
        filename_res = sprintf('cen_auc.mat');
        load(fullfile(stats_folder,filename_res));
        
        param = params_comp_all.auc;
    end
    ...
    switch rois
        ...
        case 'V123'
            t = table(squeeze(param(1,1,:)),squeeze(param(1,2,:)),squeeze(param(1,3,:)),squeeze(param(2,1,:)),squeeze(param(2,2,:)),squeeze(param(2,3,:)),squeeze(param(3,1,:)),squeeze(param(3,2,:)),squeeze(param(3,3,:)),...
                   'VariableNames',{'Y1','Y2','Y3','Y4','Y5','Y6','Y7','Y8','Y9'});
            
            
            within = table({'sf3';'sf3';'sf3';'sf6';'sf6';'sf6';'sf12';'sf12';'sf12';},{'V1';'V2';'V3';'V1';'V2';'V3';'V1';'V2';'V3'},'VariableNames',{'condition','roi'});
            rm = fitrm(t,'Y1-Y9~1','WithinDesign',within);
            ranova(rm, 'WithinModel','condition*roi')
            
            checkRoiDifference = 0;
